 ...
 * @param filePathOrUrl - The string containing the file path, URL, or
 * file name.
 * @returns The file extension (including the dot) or an
 * empty string if not found.
 */
export const getFileExtension = (filePathOrUrl: string): string => {
  const fileExtensionMatch = filePathOrUrl.match(/(\.[^./\\]*)$/);
  return fileExtensionMatch && fileExtensionMatch.length > 0
    ? fileExtensionMatch[0]
    : '';
};
